...
3. **`conftest.py` is for pytest wiring only** — fixtures registered as `@pytest.fixture`, session-scoped setup, plugin hooks. It must not contain factory logic; it wraps factories from `tests/fixtures/`. Add `conftest.py` only when a pytest fixture is genuinely needed; don't create it preemptively. Default location is `tests/conftest.py`. **Repo-root `conftest.py` is permitted only for environment setup that must run before package imports** (e.g., setting `KMP_DUPLICATE_LIB_OK` before `statepace/filter.py` imports torch); pytest discovers the root conftest before importing any test module, while `tests/conftest.py` runs only after collection has begun. The root conftest must be minimal — env vars only, no fixtures.
